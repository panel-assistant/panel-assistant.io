...
- **[Shelly Wall Display](/hardware/panels/shelly/#access-model).** No user-facing adb or root. The legacy firmware targets a `userdebug` base build, so `adb root` may work if an adb connection can be established; nothing establishes the same for current models.
